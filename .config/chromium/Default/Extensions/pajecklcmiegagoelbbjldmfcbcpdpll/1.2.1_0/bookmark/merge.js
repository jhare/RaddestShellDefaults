...
function merge_node(){
	if(merge_buffer.node_stack.length > 0){
		...
		if(local_node != null) {
			print_msg("Bookmark Not Found in Local List: [SID]=["+node.id+"], [TITLE]=["+ node.title +"],"+"[URL]=["+node.url+"]");
			...
			else
			{
				...
				if (tmp_pnode != null){
					...
					if(local_node && !local_node.hasOwnProperty('sid')) {
						print_msg("Bookmark Local Exists And Merged: [SID]=["+node.id+"], [TITLE]=["+ node.title +"],"+"[URL]=["+node.url+"]");
						local_node.sid = node.id;
						local_node.order = node.order;
					...
					{
						print_msg("Bookmark Local Not Exists And Insert: [SID]=["+node.id+"], [TITLE]=["+ node.title +"],"+"[URL]=["+node.url+"]");
						if(local_node)
						{
							print_msg("Local Not Exists");
						}
						else
						{
							print_msg("Has SID");
