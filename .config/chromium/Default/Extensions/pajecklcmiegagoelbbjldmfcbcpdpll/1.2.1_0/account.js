...
/*
	Handler for login window shading.
*/
var dolphin_login_shade_handler;

/*
	Handler for login error.
*/
var dolphin_login_error_handler;

/*
	Handler for login window close.
*/
var dolphin_loginwin;

/*
	Object of third party login window.
*/
...
/*
	Third party login entry.This function will create a new popup window and load 
	third-party auth page, login result will be checked by tab update listener.
	
	Params:
		type: one of ['facebook','google']
*/
function thirdParty_login(type, win)
{
	...
	//if there is a login process,then return. 
	...
	chrome.tabs.query({
		...
		function(tabs){	
			CommInfo.lastTabId = tabs[0].id; 
		}
	);
		
	chrome.tabs.create({
            url: API.signup + ThirdParty[type]+"?r=US&cid=" + get_clientid(false) +"&display=page&v=2&t=" + get_os_type()+"23",
            active: false
        },
		function(tab) {
...
/*
	User for third-party login status check.Server will redirect the url of login window,
	so, we need to check the change of url on current login window, if the url pattern is matched,
	then get the params from url, and save [login type,token,email], then close the login window.
	
	Params:
		this is chrome extension interface, params is a callback function which will be called 
		when the url of login window open tab changed
*/				
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
	if( 
		CommInfo.auth_tab_id != 'undefined' && 
...
function get_user_info_result(xhr)
{
	if(xhr.readyState == 4)
	{
		...
		{	
			//CommInfo.nick_name = resp.data.nick_name;
		...
		{
			//showNotification("Conncect Failed:","*.*");
		}
	}	
}

/*
	Dolphin account login.
	Params:
		username:
		password:
		shade_ctrl: handler of shade control
		show_error: handler of error display
		win_close: handler of close login window
*/
